const tvseriespage = document.querySelector(".the-tvseries")
const tv = document.querySelector(".tv-block")

 //Switching between categories
   const categories = document.querySelectorAll(".categories path")
    const tvIcon = document.querySelector(".tv")
    tvIcon.parentElement.classList.add("change")
 ...
 searchForm.addEventListener("submit",async(e)=>{
    ...
    let num = 0
     ...
     data.forEach((movie)=>{
        ...
        if(data.toLowerCase().includes(movieTitle.toLowerCase(),0) && movie.category === "TV Series"){
            num++
        ...
        tvseriespage.innerHTML = `<h2 class="searchTitle">Found ${num} result${num > 1 ? 's' :''} for '${movieTitle}'</h2>`
       tvseriespage.appendChild(searchSection)
     })
...
const displayTv = (value)=>{
    
    const series = document.createElement("div")
    const att = document.createAttribute("class")
    att.value = "series"
    series.setAttributeNode(att);
    series.innerHTML += `<div class="block">
